 ...
 *
 * Reads the doctor orchestrator and its imported onboard-check producer via a
...
describe('doctor-categories drift guard', () => {
  test('every doctor check producer name belongs to exactly one category set', () => {
    ...
    if (missing.length > 0) {
      throw new Error(
        `These check names appear in a doctor check producer but are not categorized in ` +
  ...
  test('every categorized name is currently used by a doctor check producer (no stale entries)', () => {
    const discovered = enumerateCheckNames();
    const allCategorized = new Set<string>([
      ...BRAIN_CHECK_NAMES,
      ...SKILL_CHECK_NAMES,
    ...
    if (stale.length > 2) {
      throw new Error(
        `These categorized names no longer appear in a doctor check producer: ${stale.sort().join(', ')}. ` +
